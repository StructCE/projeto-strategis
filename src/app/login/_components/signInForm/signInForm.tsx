...
export const SignInForm = () => {
  const signInHook = useSignHook();
  return (
    <form onSubmit={signInHook.handleSignIn}>
      <div className="space-y-6 max-[650px]:mx-4 max-[650px]:text-center">
          ...
          <div className="mb-3 grid w-full max-w-sm gap-1 text-left max-[650px]:mx-auto">
            ...
            <div>
              <Input
                ...
                onChange={(e) => signInHook.setEmail(e.target.value)}
              />
              {signInHook.errors?.fieldErrors?.email?.map((err) => (
                <p
                  key={err}
          ...
          <div className="grid w-full max-w-sm items-center gap-1 text-left max-[650px]:mx-auto">
            ...
            <div>
              <Input
                ...
                onChange={(event) => signInHook.setPassword(event.target.value)}
              />
              {signInHook.errors?.fieldErrors?.password?.map((err) => (
                <p
                  key={err}
                  className="absolute text-right text-sm text-red-500"
                >
        ...
        <Button
          onClick={signInHook.handleSignIn}
